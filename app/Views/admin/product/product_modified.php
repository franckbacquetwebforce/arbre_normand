...
<div class="container-fluid modification">
	<div class="row">
	  <form class="responsive modif-form" action="<?= $this->url('admin_product_update_action',['id' => $product['id']])?>" method="post">
		    ...
		    <input type="text" name="product_name" class="form-control" value="<?php if(!empty($_POST['product_name'])){echo $_POST['product_name'];}else{if(!empty($product['product_name'])){echo $product['product_name'];}} ?>"><br/>

		    <label for="description"><h4>Description</h4></label>
		    <span class=""><?php if(!empty($error['description'])){echo $error['description'];} ?></span><br />
		    <textarea name="description" rows="12" cols="80" class="form-control"><?php if(!empty($_POST['description'])){echo $_POST['description'];}else{if(!empty($product['description'])){echo $product['description'];}} ?> </textarea><br />

			</div>
			<div class="col-sm-2">
		    <label for="price_ht"><h4>Prix HT :</h4></label>
		    <span class=""><?php if(!empty($error['price_ht'])){echo $error['price_ht'];} ?></span><br/>
		    <input type="text" name="price_ht" class="form-control" value="<?php if(!empty($_POST['price_ht'])){echo $_POST['price_ht'];}else{if(!empty($product['price_ht'])){echo $product['price_ht'];}} ?>"><br />

		    <label for="weight"><h4>Poids en kg :</h4></label>
		    <span class=""><?php if(!empty($error['weight'])){echo $error['weight'];} ?></span><br />
		    <input type="text" name="weight" class="form-control" value="<?php if(!empty($_POST['weight'])){echo $_POST['weight'];}else{if(!empty($product['weight'])){echo $product['weight'];}} ?>"><br/>

		    <label for="stock"><h4>Stock :</h4></label>
		    <span class=""><?php if(!empty($error['stock'])){echo $error['stock'];} ?></span><br />
		    <input type="text" name="stock" class="form-control" value="<?php if(!empty($_POST['stock'])){echo $_POST['stock'];}else{if(!empty($product['stock'])){echo $product['stock'];}} ?>"><br />
